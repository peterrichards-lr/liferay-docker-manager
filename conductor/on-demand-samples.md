...
- `.github/workflows/ci.yml`: Update the release job to package `references/samples/` into a `samples.zip` and upload it as a release asset.

## Implementation Steps

### 1. CI Workflow Update (`.github/workflows/ci.yml`)

- In the `release` job (on `ubuntu-latest`), add a step to zip the `references/samples/` directory.
...
- Add `download_samples(version, destination)`:
  ...
  - Use `zipfile` to extract it to the `destination` (e.g., `~/.ldm/references/samples/v{version}/`).
...
- Refine `sync_samples(paths)`:
  1. Check Local: Look for `SCRIPT_DIR / "references" / "samples"`.
...
1. **Developer Mode**: Verify `ldm run --samples` still works from the git repo using the local `references/samples/` folder.
2. **Binary Mode (Simulated)**:
   - Temporarily rename the local `references/samples/` folder.
   - Run `ldm run --samples`.
   - Verify it prompts for download and correctly populates `~/.ldm/references/samples/v1.4.0/`.
