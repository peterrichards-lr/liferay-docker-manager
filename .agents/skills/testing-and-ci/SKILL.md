...
- **You MUST `git add` before calling the wrapper — it does NOT stage for you on the happy path.** `agent_push.sh` only runs `git add .` *inside its hook-failure branch* (`scripts/agent_push.sh:28`). Its commit is then guarded on `git diff --cached --quiet`, but **`git push origin HEAD` runs unconditionally**. So when every gate passes first time and nothing is staged, the wrapper commits nothing, pushes an empty branch, prints `✅ Push completed successfully!` and exits `0`.

  The incentive is inverted, which is what makes this dangerous: **the cleaner the work, the more likely it silently commits nothing.** Work that trips a hook gets rescued by that `git add .`; work that passes every gate first time falls straight through. Observed on 2026-08-22 during LDM-#1262 — the sole signal was one line, `=> No staged changes to commit. Proceeding to push...`, buried in ~600 lines of pytest output, and a PR containing zero changes was moments from being opened.

  Two habits follow:

  - **Stage explicitly, then verify the commit landed.** Run `git add <paths>` first, and after the wrapper returns check `git log --oneline -1` and `git show --stat HEAD`. A `✅ Push completed successfully!` is *not* evidence that anything was committed.
  - **Prefer `git add <paths>` over `git add -A`.** When hooks *do* fail, the wrapper's own `git add .` stages the entire working tree, not just hook-modified files — so unrelated edits and scratch debris ride along into the commit. That is a plausible contributor to LDM-#1233 (committed scratch/debug debris). If a hook failure triggers it, inspect `git diff --cached --stat` before letting the commit stand.
