...
- **Cross-Platform Script Parity**: The `.sh` and `.ps1`/`.bat` halves of every cross-platform utility, verification and wrapper script pair -- notably `scripts/verify_e2e_refactor.sh` / `scripts/verify_e2e_refactor.ps1` -- MUST be kept in functional parity. Any hardening check, assertion or logic change applied to one MUST land in the other in the same change. Parity has so far been maintained by habit rather than by rule, which works right up until the person or agent maintaining it changes; a Windows developer running a verification script that silently checks less than its Unix twin gets a green result that means nothing.
