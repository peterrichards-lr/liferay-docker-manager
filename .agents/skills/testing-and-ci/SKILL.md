...
- **Handling Hook Failures & Missing Binaries**: If a hook fails due to a missing binary in the local environment (e.g. `semgrep` or `detect-secrets` which only run in CI), you are FORBIDDEN from using `--no-verify`. You MUST explicitly skip ONLY the failing hooks using the `SKIP` environment variable (e.g., `SKIP=semgrep,detect-secrets git commit -m "..."`). For formatting failures (e.g. `ruff format`), you MUST run the formatter locally, stage the fixed files, and try again.
