...
### **High-Performance Boot & Hardening (v2.1.31)**

- **Flexible Command Flags**: Re-engineered the CLI parser to allow global flags (like `-v` and `-y`) to be placed both before and after subcommands (e.g., `ldm prune -y` now works). This provides a more intuitive and industry-standard UX matching tools like `docker` and `git`.
