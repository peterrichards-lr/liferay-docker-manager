...
### **High-Performance Boot & Hardening (v2.1.25)**

- **Zero-Failure Upgrades**: Hardened the self-upgrade engine to use system temporary directories for downloads. This ensures that LDM can always download and verify update assets even if the installation path (e.g., `/usr/local/bin`) is not user-writable, only requesting internal elevation for the final atomic swap.
- **Internal Elevation Policy**: Formalized the "No-Sudo" policy for running LDM. The tool now strictly prohibits being run with the `sudo` prefix to protect application caches and will instead request elevated privileges internally only when performing specific system tasks like host file updates or binary replacement.
...
