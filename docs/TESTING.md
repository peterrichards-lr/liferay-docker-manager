...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Apple Silicon** | macOS 26 Tahoe | **Colima** | `29.2.1` | ![Colima](https://img.shields.io/badge/Colima-Hardening-FFAB00?style=flat-square&logo=apple) | `2.21.1-pre.4` | ✅ | [verify-apple-silicon-macos-26-tahoe-colima-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-apple-silicon-macos-26-tahoe-colima-pass.txt) |
| **Apple Silicon** | macOS 26 Tahoe | **OrbStack** | `29.4.0` | ![OrbStack](https://img.shields.io/badge/OrbStack-Hardening-00B0FF?style=flat-square&logo=apple) | `2.23.0-pre.8` | ✅ | [verify-apple-silicon-macos-26-tahoe-orbstack-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-apple-silicon-macos-26-tahoe-orbstack-pass.txt) |
| **Apple Silicon** | macOS 27 Golden Gate | **Colima** | `29.2.1` | ![Colima](https://img.shields.io/badge/Colima-Hardening-FFAB00?style=flat-square&logo=apple) | `2.23.0-pre.8` | ✅ | [verify-apple-silicon-macos-27-golden-gate-colima-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-apple-silicon-macos-27-golden-gate-colima-pass.txt) |
| **Linux Workstation** | Alpine 3.24 | **Native Docker** | `28.0.4` | ![Linux](https://img.shields.io/badge/Linux-Hardening-success?style=flat-square&logo=linux) | `2.23.0-pre.4` | ✅ | [verify-linux-workstation-alpine-3.24-native-docker-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-linux-workstation-alpine-3.24-native-docker-pass.txt) |
| **Linux Workstation** | Debian 12 | **Native Docker** | `28.0.4` | ![Linux](https://img.shields.io/badge/Linux-Hardening-success?style=flat-square&logo=linux) | `2.23.0-pre.4` | ✅ | [verify-linux-workstation-debian-12-native-docker-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-linux-workstation-debian-12-native-docker-pass.txt) |
| **Linux Workstation** | Fedora 44 | **Native Docker** | `29.7.2` | ![Linux](https://img.shields.io/badge/Linux-Hardening-success?style=flat-square&logo=linux) | `2.23.0-pre.8` | ✅ | [verify-linux-workstation-fedora-44-native-docker-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-linux-workstation-fedora-44-native-docker-pass.txt) |
| **Linux Workstation** | Rocky Linux 9.3 | **Native Docker** | `28.0.4` | ![Linux](https://img.shields.io/badge/Linux-Hardening-success?style=flat-square&logo=linux) | `2.23.0-pre.4` | ✅ | [verify-linux-workstation-rocky-linux-9.3-native-docker-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-linux-workstation-rocky-linux-9.3-native-docker-pass.txt) |
| **Linux Workstation** | Ubuntu 24.04 | **Native Docker** | `28.0.4` | ![Linux](https://img.shields.io/badge/Linux-Hardening-success?style=flat-square&logo=linux) | `2.23.0-pre.4` | ✅ | [verify-linux-workstation-ubuntu-24.04-native-docker-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-linux-workstation-ubuntu-24.04-native-docker-pass.txt) |
| **Windows PC** | Windows 11 | **Native WSL2** `WSL 2.4.4` | `29.3.0` | ![WSL2](https://img.shields.io/badge/WSL2-Hardening-blue?style=flat-square&logo=windows) | `2.23.0-pre.8` | ✅ | [verify-windows-pc-windows-11-native-wsl2-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-windows-pc-windows-11-native-wsl2-pass.txt) |
| **Windows PC** | Windows 11 PowerShell 5.1 | **Docker Desktop** | `29.4.0` | ![DockerDesktop](https://img.shields.io/badge/Docker_Desktop-Hardening-00C853?style=flat-square&logo=windows) | `2.23.0-pre.8` | ✅ | [verify-windows-pc-windows-11-powershell-5.1-docker-desktop-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-windows-pc-windows-11-powershell-5.1-docker-desktop-pass.txt) |
| **Windows PC** | Windows 11 PowerShell 7 | **Docker Desktop** | `29.4.0` | ![DockerDesktop](https://img.shields.io/badge/Docker_Desktop-Hardening-00C853?style=flat-square&logo=windows) | `2.23.0-pre.8` | ✅ | [verify-windows-pc-windows-11-powershell-7-docker-desktop-pass.txt](https://github.com/peterrichards-lr/liferay-docker-manager/blob/master/references/verification-results/verify-windows-pc-windows-11-powershell-7-docker-desktop-pass.txt) |
...
* **A package manifest that under-reports being corrected from its payload, on
  ...
  **The manifest PARSE refusal on a local `.ldmp` is no longer in this
  category, and is now asserted E2E.** LDM-#1621 put manifest verification on
  the import pipeline, and the parse refusal fires in
  `PackageVerificationStage` before `ProjectSetupStage` creates anything -- so
  it needs no network, no published release and no Liferay boot. Both scripts
  build the package themselves and assert the refusal:
  `verify_ldmp_manifest_refusal()` in the `.sh`, `Test-LdmpManifestRefusal` in
  the `.ps1`.

  Both assert the exit code **and** the reason, which is not belt-and-braces.
  Measured on 2026-09-07 against a binary without #1621: the same package
  reached `verify_runtime_environment` and exited `1` on
  `FATAL: VOLUME MOUNTING IS BROKEN`. A check that looked only at the exit code
  would have reported that as a pass.

  What is still not asserted E2E is the **recovery** and the **origin
  reporting**. Both continue past the manifest into `cmd_restore`, so exercising
  them needs a real database restore and a Liferay boot against a package the
  script would have to build from a running project. Covered by
  `ldm_core/tests/test_local_package_verification.py`, which drives the real
  ...
  The two script functions are named, rather than inlined, so their own
  assertion logic is testable without Docker or an `ldm` binary:
  `ldm_core/tests/test_verify_scripts.py` drives each half against a stub `ldm`
  that returns a chosen exit code and message, and checks the parity of the two
  halves.
