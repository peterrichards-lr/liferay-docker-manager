...
#### **First: check which CA your certificates are actually signed by**

Both fixes below depend on knowing this, and it is the step that is usually skipped. Setting `CAROOT` does **not** retrospectively re-sign certificates that already exist -- LDM reuses a certificate it finds on disk, so an export that looks correct can sit alongside certificates still signed by the Linux CA.

1. Ask `mkcert` which CA is actually in effect in WSL:

    ```bash
    mkcert -CAROOT
    ```

    If this prints a Linux path (e.g. `/home/<you>/.local/share/mkcert`) rather than your `/mnt/c/...` path, the `CAROOT` export has not taken in this shell. `ldm doctor` reports the same value.

2. Ask the certificate itself who signed it:

    ```bash
    openssl x509 -in ~/liferay-docker-certs/<your-host>.pem -noout -issuer
    ```

    The issuer names the machine and user the CA was created for. If it names your WSL distro while `mkcert -CAROOT` points at Windows, the certificate predates the change and must be regenerated.

3. Regenerate with the correct command:

    ```bash
    ldm renew-ssl
    ```

    This deletes the host's certificate and key from `~/liferay-docker-certs` and re-issues them against the CA currently in effect. Traefik picks the new files up automatically.

#### **Solution A: Sync WSL to use the Windows CA**

Point WSL's `mkcert` at the Windows trust store so that everything LDM generates is trusted by your Windows browsers.
...
5. Regenerate the certificates so they are re-issued against the Windows CA:

    ```bash
    ldm renew-ssl
    ```

    Then re-run the checks at the top of this section. If `mkcert -CAROOT` still reports a Linux path, Solution A has not taken -- use Solution B.

#### **Solution B: Trust the WSL CA from Windows instead**

*Contributed by a user running LDM under WSL, where Solution A did not hold: certificates continued to be issued against the Linux CA despite `CAROOT` being exported.*

This is the same problem solved from the opposite end. Rather than making WSL use the Windows CA, make Windows trust the CA that WSL is already using. It is the more reliable option when `mkcert` in WSL keeps falling back to its own root, and it is a **one-off** -- the CA is long-lived, so this does not need repeating per project or per certificate.

1. In WSL, find the CA directory and confirm the root certificate is there:

    ```bash
    mkcert -CAROOT
    ls "$(mkcert -CAROOT)"        # expect rootCA.pem and rootCA-key.pem
    ```

2. Copy **only** `rootCA.pem` somewhere Windows can reach. `rootCA-key.pem` is the CA's private key -- it is not needed for trust and should not be copied around.

    ```bash
    cp "$(mkcert -CAROOT)/rootCA.pem" /mnt/c/Users/<YourWindowsUsername>/Downloads/wsl-rootCA.pem
    ```

3. Register it in the Windows **Trusted Root Certification Authorities** store. In an **Administrator** PowerShell:

    ```powershell
    certutil -addstore -f "ROOT" "$env:USERPROFILE\Downloads\wsl-rootCA.pem"
    ```

    Or via the GUI: `certlm.msc` -> *Trusted Root Certification Authorities* -> *Certificates* -> right-click -> *All Tasks* -> *Import*, and select the file.

4. Restart Chrome completely (it caches the trust store) and reload the site.

> [!NOTE]
> Solution B trusts a CA whose private key lives inside your WSL filesystem. That is the same trust you already extend to `mkcert` on any development machine, but it is worth stating: anyone who can read `rootCA-key.pem` in that WSL distro can issue certificates your Windows host will trust. Remove it from the store (`certlm.msc`, or `certutil -delstore "ROOT" <thumbprint>`) if you stop using that distro.
