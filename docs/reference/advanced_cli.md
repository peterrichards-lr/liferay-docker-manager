...
- **`--vanilla`** ![Added in v2.16.0](https://img.shields.io/badge/Added%20in-v2.16.0-blue): A readable alias for `--no-seed` — same behaviour, named for the intent rather than the mechanism. It bypasses the pre-warmed seed so Liferay builds its schema and resolves bundles from scratch. *Note: it does **not** suppress `--samples` or `--snapshot`, so combining them still restores that content into an otherwise unseeded project. See [#1285](https://github.com/peterrichards-lr/liferay-docker-manager/issues/1285).*
