...
- **A host port is published only when SSL is off.** The global Traefik proxy is provisioned for SSL projects, and the subdomain above is then the way in -- such a project publishes no per-extension host port, because it does not need one. A **non-SSL** project gets no proxy at all, so each client extension publishes a host port instead and that port is the only way to reach it. The port is the one LDM allocated for that extension and is reported by `ldm status`; it is no longer rewritten (LDM-#1969 published everything landing on 8080 as a shared `28080`, so at most one extension on the whole host could bind it, and LDM-#1985 removed the mapping altogether before restoring it correctly).
