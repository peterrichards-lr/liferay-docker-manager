...
- **Current Active Task (June 30, 2026)**:
  - [x] All 6 prioritized resource reduction & database mode issues have been fully implemented, tested, committed, and pushed.
  - [x] Created all 5 pull requests on GitHub using the `create_prs.py` script.
  - [x] Resolved merge conflicts and successfully squash-merged all 5 pull requests into master.
  - [x] Fixed a bug in `ldm fix-hosts` where `BaseHandler.cmd_fix_hosts` threw `'LiferayManager' object has no attribute 'manager'`, and opened pull request PR #236.
