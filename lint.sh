...
if [ -d "$VENV_PATH" ]; then
	"$PYTHON_BIN" -m bandit -r "$SCRIPT_DIR/ldm_core/" -x "$SCRIPT_DIR/ldm_core/tests/" -s B101,B103,B108,B110,B404,B603,B607 --severity-level medium || EXIT_CODE=1
...
if [ -d "$VENV_PATH" ]; then
	...
	"$PYTHON_BIN" -m pytest "$SCRIPT_DIR/ldm_core/tests/" --ignore="$SCRIPT_DIR/ldm_core/tests/test_e2e_interactive.py" || EXIT_CODE=1
