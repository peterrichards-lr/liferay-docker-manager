...
try {
    ...
    # UI Test
    $uiTest = "import os, pytest; from playwright.sync_api import Page, expect; def test_fragment_deployment(page: Page): url = os.environ.get('LIFERAY_URL', 'http://localhost:8082'); page.goto(f'{url}/c/portal/login'); if page.locator('input[name*=\`'LoginPortlet_login\`']').is_visible(timeout=5000): page.fill('input[name*=\`'LoginPortlet_login\`']', 'test@liferay.com'); page.fill('input[name*=\`'LoginPortlet_password\`']', 'test'); page.click('button[type=\`'submit\`']'); page.wait_for_url(f'{url}/web/guest**', timeout=30000); page.goto(f'{url}/group/guest/~/control_panel/manage?p_p_id=com_liferay_fragment_web_portlet_FragmentPortlet'); col = page.locator(':text(\`'Test Collection\`')').first; col.click(force=True); expect(page.get_by_text('Test Fragment').first).to_be_visible(timeout=10000)"
