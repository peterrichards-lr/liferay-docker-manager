...
if (cd collision-test/nested && echo "n" | "$LDM_CMD" run "collision-test" --port 8099 --no-wait --no-up --no-seed 2>&1 | grep -qE "Project collision|already registered"); then
    ...
    (cd collision-test/nested && echo "n" | "$LDM_CMD" run "collision-test" --port 8099 --no-wait --no-up --no-seed 2>&1) | tee -a "$RESULTS_FILE_TMP"
    exit 1
fi
