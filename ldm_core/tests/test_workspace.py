...
class TestWorkspaceScanners(unittest.TestCase):
    ...
    def test_rewrite_oauth_urls_in_zip_non_ssl_localhost(self):
        """LDM-#1985: the URL carries the resolved port, and it is now right.

        This has been inverted twice. It originally asserted
        `http://localhost:8082` from `meta[port_my-ext]`, which was WRONG at
        the time -- the composer rewrote 8080 to 28080 before publishing, so
        the URL baked into the extension's own OAuth configuration named a
        port nothing listened on. LDM-#1973 dropped the port to escape that.

        Dropping it was right only where a Traefik proxy serves the bare
        subdomain, and a non-SSL project has none. With the rewrite deleted
        the resolved port IS the published port, so naming it here is accurate
        and is the only address that works without a proxy.
        """
        import yaml
...
class TestWorkspaceQuickstart(unittest.TestCase):
    ...
    @patch("ldm_core.handlers.assets.AssetService._fetch_seed")
    @patch("ldm_core.handlers.runtime.RuntimeService.cmd_run")
    def test_cmd_quickstart_custom_name(
        self,
        mock_cmd_run,
        mock_fetch_seed,
        mock_setup_paths,
        ...
        # Simulate setting the --name argument via CLI args parser mock
        self.manager.args.name = "my-custom-aica"
