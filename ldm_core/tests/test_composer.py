...
class TestComposerService(unittest.TestCase):
    ...
    def test_build_liferay_service_client_extensions_bind_mount_by_default(self):
        # LDM-#1083: the default must stay a live host bind-mount, so
        # hot-deploying a new client extension in is picked up immediately --
        # this is the behavior for every project that doesn't set
        # internal_state.
        paths = {
            "root": Path("/tmp/proj"),
            "deploy": Path("/tmp/proj/deploy"),
        ...
        self.assertTrue(
            any(
                v.startswith(
                    f"{Path('/tmp/proj/osgi/client-extensions').as_posix()}:/opt/liferay/osgi/client-extensions"
                )
                for v in volumes
            )
        )
        self.assertFalse(any(v.startswith("proj-cx:") for v in volumes))

    def test_build_liferay_service_client_extensions_named_volume_when_internal_state(
        self,
    ):
        # LDM-#1083: an external/slow drive (or explicit opt-in) trades the
        # live bind-mount for a Named Volume -- this is what actually fixes
        # the confirmed root cause of #1083 (OSGi client-extension bundles
        # never reaching STARTED when Liferay's own hot-deploy watcher reads
        # this directory over slow external-drive I/O).
        paths = {
            "root": Path("/tmp/proj"),
            "deploy": Path("/tmp/proj/deploy"),
            "files": Path("/tmp/proj/files"),
        ...
        volumes = service["volumes"]
        self.assertIn("proj-cx:/opt/liferay/osgi/client-extensions", volumes)
        self.assertFalse(
            any(
                v.startswith(f"{Path('/tmp/proj/osgi/client-extensions').as_posix()}:")
                for v in volumes
            )
        )
