...
class TestSnapshotService(unittest.TestCase):
    ...
    def test_dehydrate_includes_cx_when_internal_state(self):
        # LDM-#1083: dehydration (snapshot time) has no OAuth-rewrite
        # ordering concern -- "cx" joins data/state whenever internal_state
        # is on.
        paths = {
            "root": self.test_dir,
            "data": self.test_dir / "data",
        ...
        with (
            patch.object(
                self.manager,
                "read_meta",
                return_value={"internal_state": "true", "container_name": "proj"},
            ),
            patch.object(self.manager.snapshot.volumes, "_sync_volume") as mock_sync,
        ):
            self.manager.snapshot.volumes._dehydrate_named_volumes(paths)
            self.assertEqual(mock_sync.call_count, 3)
            mock_sync.assert_any_call(paths["cx"], "proj-cx", direction="from_volume")

    def test_hydrate_cx_volume_if_internal_noop_by_default(self):
        # No internal_state set -> must not touch the volume at all, since
        # the default is a live host bind-mount for client-extensions.
        paths = {"root": self.test_dir, "cx": self.test_dir / "cx"}
        self.manager.composer.is_using_named_volumes.return_value = True
        ...
        with (
            ...
            self.manager.snapshot.volumes.hydrate_cx_volume_if_internal(paths)
            mock_sync.assert_not_called()

    def test_hydrate_cx_volume_if_internal_syncs_when_active(self):
        paths = {"root": self.test_dir, "cx": self.test_dir / "cx"}
        paths["cx"].mkdir()
        self.manager.composer.is_using_named_volumes.return_value = True
        ...
        with (
            ...
            mock_sync.assert_called_once_with(
                paths["cx"], "proj-cx", direction="to_volume"
            )

    def test_hydrate_cx_volume_if_internal_noop_without_named_volumes(self):
        paths = {"root": self.test_dir, "cx": self.test_dir / "cx"}
