...
class LDMHelpFormatter(argparse.RawDescriptionHelpFormatter):
    ...
    def format_help(self):
        root_sec = getattr(self, "_root_section", None)
        items_len = len(root_sec.items) if root_sec is not None else 0
        if self._prog != "ldm" or items_len <= 1:
...
def get_parser():  # noqa: PLR0915
    ...
    status.add_argument(
        "--json",
        action="store_true",
        help="Emit a stable, machine-readable JSON object instead of the formatted tables",
    )
