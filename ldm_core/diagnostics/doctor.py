...
class DoctorRunner:
    ...
    def _check_tooling_and_integrity(self):  # noqa: C901, PLR0912, PLR0915
        ...
        if latest and version_to_tuple(latest) > version_to_tuple(VERSION):
            ...
            self.add_hint(
                ...
                f"{GITHUB_DOCS_URL}/INSTALLATION.md#troubleshooting-version-loop--integrity-issues",
            )
        else:
        ...
        if is_in_venv:
            ...
            self.add_hint(
                ...
                f"{GITHUB_REPO_URL}/blob/master/GEMINI.md#6-python-virtual-environment-venv",
            )
        ...
        if is_completion_enabled(self.handler):
            ...
            if shell in ["bash", "zsh", "fish"]:
                ...
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/INSTALLATION.md#3-shell-autocompletion",
                )
            else:
                self.results.append(
    ...
    def _check_gui_dependencies(self):
        ...
        if sys.platform.startswith("linux"):
            ...
            self.add_hint(
                ...
                f"{GITHUB_DOCS_URL}/reference/advanced_cli.md",
            )
            self.add_hint(
        ...
        if self.docker_version:
            ...
            try:
                ...
                if getattr(self, "_colima_mount_not_writable", False):
                    self.add_hint(
                        ...
                        f"{GITHUB_DOCS_URL}/INSTALLATION.md#colima-mount-permissions",
                    )
                    self.add_hint(
                ...
                if self.is_wsl:
                    ...
                    if socket_path.is_symlink():
                        ...
                        self.add_hint(
                            ...
                            f"{GITHUB_DOCS_URL}/INSTALLATION.md#wsl2-mixed-environments",
                        )
                        self.add_hint(
            ...
            if self.is_wsl:
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/INSTALLATION.md#wsl2-native-docker",
                )
                self.add_hint(
    ...
    def _check_global_config_and_network(self):  # noqa: C901, PLR0912, PLR0915
            ...
            try:
                ...
                if "Permission denied" in str(res) or "Read-only file system" in str(
                    ...
                    self.add_hint(
                        ...
                        f"{GITHUB_DOCS_URL}/INSTALLATION.md#troubleshooting-read-only-mounts",
                    )
                        ...
                        if (
                            ...
                            if is_intel:
                                ...
                                self.add_hint(
                                    ...
                                    f"{GITHUB_DOCS_URL}/colima-performance-tuning",
                                )
        ...
        if mkcert_ok is not True:
            if mkcert_status == "Not installed":
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/INSTALLATION.md#prerequisites",
                )
                ...
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/INSTALLATION.md#fixing-ssl-trust-issues-mkcert",
                )
        else:
            # Detect WSL
            self.is_wsl = False
            ...
            if self.is_wsl:
                ...
                if not is_win_ca:
                    self.add_hint(
                        ...
                        f"{GITHUB_DOCS_URL}/INSTALLATION.md#wsl2-ssl-trust",
                    )
                else:
                    self.add_hint(
        ...
        if not common_dir.exists():
            ...
            self.add_hint(
                ...
                f"{GITHUB_DOCS_URL}/INSTALLATION.md#global-configuration-the-common-folder",
            )
        else:
            try:
        ...
        if self.docker_version:
            ...
            # LDM-#1090/#1133/#1165: shared infra resolves to whichever
            # target is active for this invocation (a project on a remote
            # node has its shared search/proxy *on that node*, not always
            # local -- see docs/explanation/remote-node-architecture.md
            # §5), so these health checks must ask the same engine
            # setup_global_search()/setup_global_database() actually use,
            # not always the local daemon.
            ...
            for container, label in global_services:
                ...
                if is_running:
                    ...
                    if container == "liferay-docker-proxy":
                        # Deliberately always local, unlike the rest of this
                        # loop: this macOS-only socket bridge (see the
                        # platform.system() == "darwin" guard above where
                        # this entry is added) is a concept specific to
                        # *this* orchestrating machine's Docker Desktop/
                        # Colima setup -- a remote Linux target has no
                        ...
                        try:
                            ...
                            if search_res not in {"200", "401"}:
                                ...
                                watermark_status = _check_elasticsearch_watermarks(
                                    self.handler,
                                    self.add_hint,
                                    docker_prefix=infra_docker_prefix,
                                )
                                if watermark_status:
        ...
        for p_path in self.project_paths:
            ...
            if poisoned:
                ...
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/README.md#env",
                )
            else:
            ...
            if self.handler.manager.require_compose(p_path, silent=True):
                ...
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/README.md#run-alias-up",
                )
            ...
            if pe_file.exists():
                ...
                if prop_ok is not True:
                    self.add_hint(
                        ...
                        f"{GITHUB_DOCS_URL}/LDM_ARCHITECTURE.md#5-metadata--property-injection",
                    )
                    if prop_details:
                        for detail in prop_details:
            ...
            if liferay_container:
                ...
                if log_ok is not True:
                    self.add_hint(
                        ...
                        f"{GITHUB_DOCS_URL}/README.md#logs",
                    )
            ...
            if use_shared_search:
                ...
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/README.md#migrate-search",
                )
            ...
            if lic_ok is not True:
                self.add_hint(
                    ...
                    f"{GITHUB_DOCS_URL}/LDM_ARCHITECTURE.md#key-architectural-pillars",
                )
            if lic_details:
                ...
                if traefik_conf.exists():
                    ...
                    if expected_cert in conf_content and expected_key in conf_content:
                        ...
                        self.add_hint(
                            ...
                            f"{GITHUB_DOCS_URL}/README.md#ssl-defaults-new-projects",
                        )
                else:
                    self.results.append(
                ...
                try:
                    ...
                    if not has_net_label:
                        ...
                        self.add_hint(
                            ...
                            f"{GITHUB_DOCS_URL}/INSTALLATION.md#dns--subdomain-configuration",
                        )
                        ...
                        self.add_hint(
                            ...
                            f"{GITHUB_DOCS_URL}/README.md#command-reference",
                        )
                    else:
                        self.results.append(
                    ...
                    try:
                        ...
                        if token_val in (verify_res or ""):
                            ...
                            self.add_hint(
                                ...
                                f"{GITHUB_DOCS_URL}/INSTALLATION.md#the-ghost-mount-issue",
                            )
                    finally:
    ...
    def _check_absolute_disk_space(self, docker_prefix=None):
        ...
        `docker_prefix` (e.g. `["docker", "--context", "aws-1"]`) redirects
        this to a remote target's engine -- disk space is a property of
        whichever engine you're asking, so an explicit --node/--target
        should reflect that engine's disk, not always the local one.
        """
    ...
    def _check_dangling_and_print(self):  # noqa: C901, PLR0912, PLR0915
        ...
        # explicit --node/--target's engine instead of always the local
        # one, matching _check_docker_runtime's own remote redirect.
        explicit_target_name = getattr(self.manager, "target", None)
        docker_prefix = None
        if explicit_target_name and explicit_target_name != "local":
            from ldm_core.config import get_active_target
...
def _check_elasticsearch_watermarks(self, add_hint, docker_prefix=None):
            ...
            for decider in node.get("deciders", []):
                if (
                    ...
                    add_hint(
                        ...
                        f"{GITHUB_DOCS_URL}/TROUBLESHOOTING.md#disk-space-issues-elasticsearch-flood-stage",
                    )
                    add_hint(
                ...
                if "flood stage disk watermark" in line_lower:
                    if add_hint:
                        add_hint(
                            ...
                            f"{GITHUB_DOCS_URL}/TROUBLESHOOTING.md#moving-docker-to-an-external-drive-macos--colima",
                        )
                    continue
