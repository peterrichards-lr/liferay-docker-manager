...
- **Next Focus: Roadmap Execution & CLI Namespacing**
  ...
  - [ ] **Windows PowerShell Input Hang Fix**:
    - [ ] Refactor `UI.ask` in `ldm_core/ui.py` to use native `input(prompt)` on Windows (`sys.platform == "win32"`) with a safe ASCII prompt, bypassing buffer-related console hangs.
    - [ ] Add unit tests in `ldm_core/tests/test_ui.py` to verify `UI.ask` and `UI.confirm` behavior on Windows vs Unix.
